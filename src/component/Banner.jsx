...
const Banner = () => {
  ...
  const fechData=async()=>{
      try {
        ...
        const list= await fetch('https://www.swiggy.com/dapi/restaurants/list/v5?lat=16.69560&lng=74.23170&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING');
            ...
            const restaurants =
                  dt?.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle?.restaurants || [];
                  setData(restaurants);
  ...
  const handleSearch = (e) => {
    ...
    setSearchData(value);
    const searchList = data.filter((el) => el.info.name.toLowerCase().find(value));
    setFilterlist(searchList);
   
  };
  const online=useOnlineStatus();
  if(!online) return <h1 className="bg-black py-2 px-4 text-white absolute bottom-3 left-2/4 -translate-x-2/4">your internet not working please check it</h1>;
  ...
  return data.length === 0 ? <Shimmer />:(
    <div className="container max-w-6xl mx-auto px-4 pt-16">
     ...
     <div className="w-full md:w-2/4 h-fit rounded-sm border shadow-md">  
      ...
      /></div>
         <button className="bg-yellow-400 py-2 px-2 rounded-sm w-fit md:w-auto m-auto md:m-0 shadow-md" 
               onClick={()=>{
          let filtList=data.filter((ele)=>ele?.info?.avgRating > 4);
          setData(filtList);

        }}>top rating food</button>
