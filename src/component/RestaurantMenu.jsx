...
const RestaurantMenu=()=>{
   ...
   const {name,city,costForTwo,totalRatingsString,avgRating} =resInfo?.cards[2]?.card?.card?.info || {};
   const datacard=resInfo?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR.cards;
  const category=datacard.filter((li)=>li?.card?.card?.["@type"]==="type.googleapis.com/swiggy.presentation.food.v2.ItemCategory");
    ...
    return (
        ...
        <div className="flex gap-1 text-center text-gray-950 font-semibold justify-center">
            ...
            </span>{avgRating} ({totalRatingsString}) .<span> {costForTwo/100}rs</span>
            </div>
